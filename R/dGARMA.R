...
dGARMA <- function(x,
  ...
  if (cc == m)                   {
    ...
    warning('All values are set as conditional values --- output density is set equal to one by convention');
  ...
  for (i in 1:n) {
    ...
    if (sum(MM) == 0) {
      OUT[i] <- 0;
      warning('All non-conditional elements in row ', i, ' of input are missing (marginal) values --- output density is set equal to one by convention'); }
    if (sum(MM) == 1) {
      OUT[i] <- dnorm(x     = XX[MM],
                      mean  = CMEAN[MM],
                      sd = CVAR[MM,MM],
